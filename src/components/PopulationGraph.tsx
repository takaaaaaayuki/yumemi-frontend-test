...
const PopulationGraph: React.FC<Props> = ({ populationDataList, selectedPopulationType }) => {
  ...
  // すべての年ごとに、人口データをまとめるための基準（年の配列）を作成
  const years: number[] = populationDataList[0].data.data[0].data.map((d) => d.year);
  ...
  const chartData = years.map((year: number, index: number) => {
    ...
    populationDataList.forEach((item) => {
      const targetDataEntry = item.data.data.find((d) => d.label === selectedPopulationType);
      if (targetDataEntry) {
  ...
  // 各都道府県に、上のカラーパレットから色を順番に割り当てる
  const getColor = (index: number): string => {
    return colorPalette[index % colorPalette.length];
  };

  // ツールチップ（グラフにマウスを合わせたときに出る情報）のデザインをカスタマイズ
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      >
        ...
        >
          <ResponsiveContainer width="100%" height={isMobile ? 300 : 400} data-testid="recharts-container">
            <LineChart
              data={chartData}
              margin={{
                top: 20,
      ...
      {/* 表示中のデータ内容（対象期間や人口の種類）をテキストでまとめて表示 */}
      <div
        style={{
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <span style={{ fontWeight: '500', color: '#1f2937' }}>{selectedPopulationType}</span>
          </div>
        </div>
      </div>
    </div>
